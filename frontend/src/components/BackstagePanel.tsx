    ...
    <div className="w-full mt-8 animate-accordion-down">
      <div className="mb-6">
        <h3 className="text-xl font-serif text-foreground mb-2">
          Multi-agents with MCP in action
        </h3>
        <p className="text-sm text-muted-foreground font-sans">
          MCP lets AI agents connect to external data sources. Agents were given access to the MoSPI MCP server and its tools.
          Here’s the agent flow that produced the result:
        </p>
      </div>
...
function StepCard({ step }: {step: MCPStep;}) {
  ...
  const hasRaw = Boolean(step.rawJson);
  return (
